...
class Neo4jService:
    ...
    def create_edge_cypher(edge):
        ...
        return f'MATCH (b:{dest["node"]}), (a:{source["node"]}) WHERE a.{source["key"]} = ${source["key"]} AND b.{dest["key"]} = ${dest["key"]} OPTIONAL MATCH (a)-[r:{edge["label"]}]->(b) WHERE r IS NULL CREATE (a)-[:{edge["label"]}]->(b)'
